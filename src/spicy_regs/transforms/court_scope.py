...
def court_jurisdictions(
    *, dump_date: date | None = None, local_file: Path | None = None
) -> dict[str, str]:
    ...
    reader = CourtListenerBulkReader(
        COURTS_DATASET, dump_date=dump_date, local_file=local_file
    )
    codes = {
        row["id"]: (row.get("jurisdiction") or "")
...
def build_docket_court_map(
    ...
    *,
    ...
    logger.info(
        "Court scope: docket->court map has {:,} dockets ({:.2f} GiB compressed read, "
        "{} resume(s)) -> {:.1f} MiB",
        written,
