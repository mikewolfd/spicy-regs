...
inside ``build_court_opinion_clusters``. A run streams two dumps, not one: the
2.3 GiB ``opinion-clusters`` dump plus the 4.67 GiB ``dockets`` dump, because
the cluster dump says nothing about which court decided — that lives on the
docket. The docket map is cached by dump date, so re-running inside one quarter
pays once.
"""
