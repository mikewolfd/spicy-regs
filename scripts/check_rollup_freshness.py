...
SKIPPED = {
    ...
    # The print-citation families. Neither document table is a daily-watermark
    # table: House activity reports are published once a Congress and budget
    # volumes once a fiscal year, so any age budget a daily check could use
    # would page for months at a time on a source behaving normally. The two
    # derived tables carry no watermark of their own at all. Promote the two
    # document tables to ROW_CHANGE_BUDGETS once a first publish lands and a
    # steady-state row count is known — a stalled rollup shows there, where a
    # date budget cannot show it.
