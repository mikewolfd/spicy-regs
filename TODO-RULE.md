...
6. Apply the same profile and projection to every remaining corpus family.
7. Complete `RULE-025` through `RULE-028` and prove the primary product goal.
8. Expand the learning loop only after the shared assertion and evidence model
...
- [ ] Generate candidates from shared concepts, entities, authorities,
      ...
      fragments.
- [ ] Add embedding similarity only after identity, graph, and concept signals.
...
tests, not a new tagging architecture.
