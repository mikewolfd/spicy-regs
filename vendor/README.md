...
- `spicy_docs-0.26.6`: built from SpicyDocs `main` at `6673fa3413cb8887057ac2b262dc4b0b2bb88083`, with
  ...
  footers, and is the same wheel DocSpec, RefSpec, SpicySearch and SpicyEngine
  pin. Its 271 packaged source files are byte-identical to the previous
  `spicy_docs-0.26.5` entry below; only the version and the Rulespec Artifacts
  pin change. Qualified by the SpicyDocs and SpicyRegs gates.

- `rulespec_artifacts-1.1.1`: exact dependency of SpicyDocs 0.26.6, built from
