...
def _fixtures(tmp_path: Path) -> tuple[Path, Path, Path]:
    ...
    pq.write_table(
        pa.Table.from_pylist(
            ...
            schema=pa.schema(
                [("cl_docket_id", pa.string()), ("court_id", pa.string())]
            ),
        ),
        docket_map,
    )
...
def test_full_mode_keeps_every_original_column_and_the_published_order(
...
def test_auto_falls_back_to_the_side_table_when_the_rewrite_would_cross_the_floor(
    tmp_path: Path, monkeypatch
):
