...
def _snapshot(snapshot_id: str, *, format_version: int, artifacts: dict) -> dict:
    ...
    Both documents live under ``materialized/rulemaking/snapshots/<id>/``, which
    ...
    return {
        "https://r2.example/materialized/rulemaking/latest.json": {
...
def test_resolves_every_table_from_one_snapshot_manifest() -> None:
    ...
    pointer_url = "https://r2.example/materialized/rulemaking/latest.json"
    manifest_key = f"materialized/rulemaking/snapshots/{snapshot_id}/manifest.json"
    ...
    documents = {
        ...
        manifest_url: {
            ...
            "snapshot_id": snapshot_id,
            "artifacts": {
                f"{table}.parquet": {"remote_key": (f"materialized/rulemaking/snapshots/{snapshot_id}/{table}.parquet")}
            ...
            "artifacts": {
                f"{table}.parquet": {
                    "remote_key": (
                        ...
                        else f"materialized/rulemaking/snapshots/{snapshot_id}/{table}.parquet"
                    )
                }
                for table in MATERIALIZED_TABLES
            },
    ...
    def fetch(url: str) -> dict:
        ...
        return {
            ...
            "artifacts": {
                f"{table}.parquet": {
                    "remote_key": (
                        f"materialized/rulemaking/snapshots/snapshot_other/{table}.parquet"
                        if table == "proceedings"
                        else f"materialized/rulemaking/snapshots/{snapshot_id}/{table}.parquet"
                    )
                }
...
def test_a_pointer_and_manifest_that_disagree_on_version_are_refused() -> None:
    ...
    documents["https://r2.example/materialized/rulemaking/latest.json"]["format_version"] = 1

    with pytest.raises(RuntimeError, match="manifest"):
        resolve_materialized_table_urls("https://r2.example", fetch_json=documents.__getitem__)


def test_format_versions_match_the_producer() -> None:
    """The reader's version tuple is a mirror of the pipeline's, kept equal here."""
    from spicy_regs.pipelines import materialized

    assert SUPPORTED_FORMAT_VERSIONS == materialized.SUPPORTED_FORMAT_VERSIONS


def test_rulemaking_tables_match_the_pipeline_outputs() -> None:
    """Every table the rulemaking pipeline publishes is one the reader resolves, and no other."""
    from spicy_regs.pipelines.rulemaking_dataset import RulemakingDatasetPipeline
    from spicy_regs.published import MATERIALIZED_DATASETS, materialized_dataset

    published = {name.removesuffix(".parquet") for name in RulemakingDatasetPipeline.published_outputs}
    assert MATERIALIZED_DATASETS["rulemaking"] == published
    assert all(materialized_dataset(table) == "rulemaking" for table in published)
    assert materialized_dataset("dockets") is None
