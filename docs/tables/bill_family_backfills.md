...
**Coverage.** Not a range. Processing state, not published data: one row per bill the `bill-family` rollup has attempted from the API detail route for a Congress below the 108th. None yet — the first run that names a pre-108th Congress fills it. It describes this pipeline's own runs, not the publisher's corpus, so no coverage range applies to it. *(measured 2026-09-19)*
