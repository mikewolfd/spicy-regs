...
**Data quality.** The bill linkage has one publisher source: `bill_action_recorded_vote` is a `recordedVotes` entry on the bill's own action, read from `bill_vote_references`, and the earliest action naming a roll call wins. Every other row is `unmatched`, including a roll call no bill's action records, whose Clerk file may still print a `legis-num`. The contract's `house_vote_legislation` rule (Congress.gov's `house-vote` listing) is no longer produced and no published row carries it: over the 115th-119th that listing named exactly the Clerk index's roll calls and linked two that no recorded reference names, 115-1-527 and -528 (H.R. 3354), so it was retired with the api.data.gov key it needed (2026-09-26). `conflict_count` is per roll call, not per run: it counts the later references that named a different bill for *this* roll call, and they are kept rather than dropped. With no bill-family run published, these columns stay unset on new rows. Each run fills a NULL `vote_day` on every held (completely captured) roll call from its own `vote_date` before merging, so a row published before the column existed gains it without a re-fetch. An incomplete capture is not held: it is re-fetched first each run and takes its day from the file. A linkage-only row keeps NULL.
