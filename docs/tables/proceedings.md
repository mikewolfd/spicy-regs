...
**Coverage.** Derived. One proceeding per independently evidenced rulemaking action, assembled in the rulemaking materialization from `rule_targets`, `federal_register` and `unified_agenda`; the population follows those inputs. Not yet published to the bucket as of the measurement date, so no row count or date range is stated here. *(measured 2026-09-18)*

- **Parquet file:** `proceedings.parquet` inside the current `materialized/rulemaking/latest.json` snapshot
