...
OLRC states no Last-Modified, ETag or Content-Length for the zip, so each run fetches it; it was byte-identical on 2026-09-14 and 2026-09-26 at the same release point. Each Congress keeps a checkpoint in the file's metadata: the derivation rule with the spicy-docs release, the member's digest and release point, and each act's shaped-row digest. While all match, nothing is derived. Otherwise every scoped act is derived again, and only an act whose rows changed is published, its whole row set replaced. Being the whole table, the file also establishes absence: a held act it no longer lists loses its rows. A failed read publishes nothing and keeps every checkpoint. `usc_section_key` is `usc_section` folded for joins (trimmed, lower-cased, every dash spelling an ASCII hyphen), because case carries no identity in the Code and Table III itself prints some sections both ways (decision 30, `docs/research/fork-delivery-decisions-2026-09-22.md`). All columns are stored as VARCHAR.
