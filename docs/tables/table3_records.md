...
**Data quality.** Table III lags enactment by tens of laws (the 119th stood at 119-73 there while the law route reached 119-110, measured 2026-09-14), and an act it does not yet hold answers a page cut off inside the site menu, which the reader refuses; the walk stops at the first such act for that Congress and resumes there next run, so the lag costs one request a run. An act's page is read once: a later edit to it (a status moving to repealed) is not picked up, and `release_point` says how current the captured page was. Private laws and pre-1957 session-law chapters are not requested. All columns are stored as VARCHAR.
