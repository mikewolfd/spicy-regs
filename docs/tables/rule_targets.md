<!-- Generated by `spicy-regs-dict generate`. Do not edit by hand. Edit data_dictionary/descriptions.yaml or the schema, then regenerate. -->

# `rule_targets`

**Rule targets**

Action-specific docket evidence: one row per docket, normalized CFR target, optional RIN, and source. Unified Agenda CFR values stay on their editioned agenda observations and are never projected through RIN equality. Corroborating sources remain separate rows. All columns are VARCHAR.

**Coverage.** Derived. Docket-to-CFR-target evidence rows built in the rulemaking materialization from `documents`, `federal_register` and Unified Agenda observations; the population follows those inputs. Not yet published to the bucket as of the measurement date, so no row count or date range is stated here. *(measured 2026-09-18)*

- **Parquet file:** `rule_targets.parquet` inside the current `materialized/rulemaking/latest.json` snapshot
- **Queryable via MCP `query_sql`:** Yes

| Column | Type | Description |
| --- | --- | --- |
| `docket_id` | `VARCHAR` | Subject regulations.gov docket. |
| `cfr_ref` | `VARCHAR` | Compact CFR key `<title>-<part>[.<section>]`; null on RIN-only edges. |
| `cfr_title` | `VARCHAR` | Decomposed CFR title number. |
| `cfr_part` | `VARCHAR` | Decomposed CFR part number. |
| `cfr_section` | `VARCHAR` | Decomposed CFR section, usually null for FR part-level metadata. |
| `rin` | `VARCHAR` | Regulation Identifier Number associated with the edge, when known. |
| `source` | `VARCHAR` | Action-evidence path: `fr_cfr_ref`, `docket_rin`, `document_rin`, or `document_fr_doc`. |
| `evidence_id` | `VARCHAR` | A concrete source row justifying the edge: FR document number, docket id, or document id. |
| `first_seen` | `VARCHAR` | Earliest publication/action timestamp among folded evidence for this logical edge. |
| `last_seen` | `VARCHAR` | Latest publication/action timestamp among folded evidence for this logical edge. |
| `method` | `VARCHAR` | Derivation method; `deterministic` in v1. |
| `actor_id` | `VARCHAR` | Versioned ruleset that produced the assertion. |
| `run_id` | `VARCHAR` | Pipeline run identifier. |
| `asserted_at` | `VARCHAR` | UTC timestamp when the row was asserted. |
| `supersedes_id` | `VARCHAR` | Prior assertion revised by this row; null for deterministic v1 spine rows. |
