...
**Coverage.** Derived. One row per continuous public-comment interval, reopenings included, read from `federal_register` documents and `dockets` in the rulemaking materialization; the population follows those inputs. Not yet published to the bucket as of the measurement date, so no row count or date range is stated here. *(measured 2026-09-18)*

- **Parquet file:** `comment_periods.parquet` inside the current `materialized/rulemaking/latest.json` snapshot
