...
**Coverage.** Sampled. No run has been measured yet, so this states the weakest true claim rather than a range it has not established. One row per member per roll call in `roll_call_votes`, so it inherits that table's House-only scope and per-run cap exactly. *(measured 2026-09-19)*
