...
| `bill_subjects` | One bill's Library of Congress subject assignment, distinct from the bill it describes | `bill_id`; no independent IRI — the assignment is a claim about the bill, not a thing | Controlled-vocabulary assertion over the `congress_bills` artifact; Congress.gov `/subjects` or GPO BILLSTATUS | S/G; one policy area plus any number of legislative subjects, both about one bill | Assignment terms, the carrier that answered, and fetch time; coverage is partial by construction | `compose`: two flat vocabularies, never a hierarchy — the carrier states policy area and subjects as siblings and no roll-up table has been obtained |
...
| `court_opinion_clusters` | One court decision (a CourtListener opinion cluster), not one authored opinion | `cluster_id`; canonical CourtListener opinion URL | Decision/container composition; CourtListener | S/G; groups the sibling opinions of one decision and carries `cl_docket_id` to `court_dockets` | Case identity, filing date, precedential status, reporter prose, docket join | `compose`; no legal-effect assertion is inferred — the decision is a container, the opinion text is the artifact |
| `court_opinion_bodies` | One authored judicial opinion within a decision, not the decision | `opinion_id`; CourtListener opinion id plus the publisher's source digest | `rkaf:Artifact`; the deciding court | A/G; a lead opinion, concurrence, or dissent is its own row, typed by `opinion_type` | Opinion role, author, body text as published (`plain_text`, `html_with_citations`), and which renderings exist | `profile`: document processing and retrieval; text provenance is recorded, never reconstructed |
