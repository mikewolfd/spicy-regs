...
| `unified_agenda` | One RIN in one agenda edition; evidence record, not a Proceeding | `(rin,agenda_edition)`; local source-record IRI | `rkaf:Artifact` with `rkaf:us-rin` evidence; Reginfo.gov | E; contributes stages, targets, authorities, and proceeding identity evidence | RIN, stage observation, CFR/legal-authority citations, timetable; source edition | Partially mapped via derived tables. `profile`: source fields retained; `Rulespec candidate`: editioned observation projection |
...
| `rule_targets` | One evidence-qualified docket/RIN/target relationship | stable tuple of subject, target, source, and evidence | Typed assertion between `rkaf:Proceeding`/`Docket` and `rkaf:Artifact` | G; joins dockets, UA, FR, documents, CFR targets | Affected citation, produced edition when known, method, evidence, first/last seen, supersession | L0 maps citation-level affected targets. `source data unavailable`: immutable pre/post-action editions remain unresolved |
| `authority_edges` | One parsed or retained authority citation for a RIN | stable tuple of RIN, raw citation, edition | `rkaf:Authority`/`Artifact`; Reginfo.gov, composed with U.S.C./Public Law identifiers | G; authority evidence attaches to proceeding through RIN | Exact/partial/failed parse, raw value, edition, provenance | L0 partial. `source data unavailable` when edition or exact section cannot be resolved; raw evidence retained |
| `proceedings` | One evidence-connected regulatory process component | `proceeding_id`; `urn:spicy-regs:proceeding:{proceeding_id}` | `rkaf:Proceeding`; derived from source-of-record evidence | G; connects RINs, dockets, artifacts, targets, authorities, predecessor/successor proceedings | Identity evidence, current stage plus event history, continuity, relationships, complete deterministic provenance | L0 partial/Experimental. `profile`: component algorithm and local ID; repaired Rulespec contract governs shared relations |
...
All 29 entries above are generated from or reconciled with the public registry.
...
`proceedings`, `comment_periods`, `concepts`, `concept_assignments`, and
`concept_events`. Its manifest binds all seven artifacts to the same source
inputs, prior state, code version, run, snapshot, row counts, and hashes.
