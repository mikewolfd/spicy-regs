...
- **Failed.** The bill family `5990abbb…` writes backward `section_diffs` when
  ...
  Both repairs are in progress; neither generation is qualified.
...
| T15 | `run-rollup-docket-search` | `docket_search.json.gz` | verified at table digest `c761cc8c…` (2026-09-26; ETag `cd719934…`): all 279,362 search documents match an independent field-by-field replay from dockets `308b35c6…`; the same 18 dockets without title or abstract are omitted. A later regeneration (ETag `7421faa6…`) is unaudited. Receipt: `drift-qualification-2026-09-26/regulatory/`. |
...
| T16 | `run-rollup-congress-bills` (retired, A1) | `congress_bills.parquet` | current `5990abbb…` is PARTIAL (2026-09-26): all 419,978 identities survive; 150 of 166 changed rows match fresh same-revision BILLSTATUS in eight fields, and 16 later-revised bills are unjudged. The generation retains no source. Retained 110th–113th BILLSTATUS lists 513 bills that `congress_bills` lacks. Scoped repair remains qualified at `4a1949a9…` (2026-09-24). Receipt: `drift-qualification-2026-09-26/bills-citations/`. |
| T11 | `run-rollup-unified-agenda` | `unified_agenda.parquet` | qualified at `b07c51d4…` (2026-09-26): the retained edition's 3,954 rows have table bytes identical to the qualified `c63a3a10…`. No new claim about source freshness. Receipt: `drift-qualification-2026-09-26/regulatory/`. |
...
| T09 | `run-rollup-bill-family` | `congress_bills.parquet`, `bill_actions.parquet`, `bill_committees.parquet`, `bill_publisher_summaries.parquet`, `bill_versions.parquet`, `bill_sections.parquet`, `section_diffs.parquet`, `section_diff_items.parquet`, `financial_changes.parquet`, `section_classifications.parquet`, `bill_summaries.parquet`, `diff_summaries.parquet`, `cbo_cost_estimates.parquet`, `public_activity_events.parquet`, `bill_family_archives.parquet`, `bill_vote_references.parquet`, `bill_family_backfills.parquet`, `bill_family_backfill_walks.parquet` | current `5990abbb…` is FAILED (2026-09-26): bytes, schemas, counts and every prior identity pass. `section_diffs` pairs five dateless enrolled bills backward (enrolled→introduced; `build_bill_family.py:580` sorts an empty BILLSTATUS date first), with eight more inherited from `4a1949a9…`; `bill_sections` drops two sections on identity collisions (119-hr-5334 enrolled, 119-hr-9022 rh), leaving a dangling diff item. 60 of 597 new bodies are byte-identical by sha256, and their sections check against independent XML reads; all 147 text diffs reconstruct. 150 of 166 changed bills match fresh same-revision BILLSTATUS. No retained source; the four model outputs remain uncomputed. Receipt: `drift-qualification-2026-09-26/bills-citations/`. |
...
| T08 | `run-rollup-print-citations` | `house_activity_reports.parquet`, `budget_volumes.parquet`, `bill_committee_actions.parquet`, `document_citations.parquet` | current `d57faac3…` is FAILED (2026-09-26): all prior cells and 68 parent recounts pass. Fourteen fresh parents reproduce their `text_sha256`; 13,219 citation spans replay exactly and pass pypdf/regex challenges. All eight new Senate reports cover the 117th Congress but stamp the 118th: 1,915 bill citations and 369 actions target the wrong bills (for example, H.R. 5376 → 118-hr-5376). MODS shares that stamp, and `a5d46aa6…` carried the same fault in six 119th reports. "CLAUSE S 2(N)" misreads as 117-s-2. Receipt: `drift-qualification-2026-09-26/bills-citations/`. |
