...
| T16 | `run-rollup-congress-bills` (retired, A1) | `congress_bills.parquet` | current `d380cdc0…` is PARTIAL (2026-09-26): all 420,166 identities survive with none added, and only update dates and related-bill fields change. All 513 bills that retained 110th–113th BILLSTATUS lists are still absent; the retired list walk left 1,299 missing across the 108th–117th (receipt `join-gaps-2026-09-26/d/`). Scoped repair remains qualified at `4a1949a9…` (2026-09-24). Receipts: `bill-family-d380cdc0-2026-09-26/`, `drift-qualification-2026-09-26/bills-citations/`. |
...
| T09 | `run-rollup-bill-family` | `congress_bills.parquet`, `bill_actions.parquet`, `bill_committees.parquet`, `bill_publisher_summaries.parquet`, `bill_versions.parquet`, `bill_sections.parquet`, `section_diffs.parquet`, `section_diff_items.parquet`, `financial_changes.parquet`, `section_classifications.parquet`, `bill_summaries.parquet`, `diff_summaries.parquet`, `cbo_cost_estimates.parquet`, `public_activity_events.parquet`, `bill_family_archives.parquet`, `bill_vote_references.parquet`, `bill_family_backfills.parquet`, `bill_family_backfill_walks.parquet` | current `d380cdc0…` is FAILED (2026-09-26): run 36219598578 at `a555f73` repairs both audited defects. No pair runs backward, 21 pairs reach an enrolled printing, and 119-hr-5334 enrolled, 119-hr-9022 reported and 119-hr-4060 introduced publish every section. The 32 stale pairs retired are exactly 18 backward and 14 that skipped the enrolled printing, and every removed item belongs to one of them. It exposes a new identity defect: a bill printed twice at one stage (119-hr-6644 `eas` and `eas2`) maps both printings to one (bill, version_code, source), so 198 section_diff_items on each side name the other printing's elements and resolve to no section. The generation audit admits every member and evidence `e3ea6ba5…`, bound to this generation, with every body of its expected kind and no credential found. Keyless vote references and archives differ only in re-observation times. The four model outputs remain uncomputed. Receipt: `bill-family-d380cdc0-2026-09-26/`. |
