...
| T08 | `run-rollup-house-communications` | `house_communications.parquet` | current `257e889a…` is PARTIAL (2026-09-26): 5,006 rows. R8 admits evidence `8ca61398…` (1,045 members, bound). An independent replay matches list membership (5,006 = listed = declared), all 34,042 list cells, and 16,000 detail cells of the 1,000 rows read this run. Limits: 6 rows are list-only past the cap, and 4,000 older details were read before evidence was retained. `a663f38` re-reads once every held detail no retained response backs; it is self-limiting and journals the remaining list. It is draining in paced runs, and the table qualifies once a generation has no list-only rows and an empty unverified list. Receipt: `amend-hc-qualification-2026-09-26/house-communications/`. |
...
- **Table III cold start:** observed 2026-09-26: OLRC's 118-273 page names 119-1 as its next act (receipt `open-work-investigation-2026-09-26/`). The real risk is scope: the laws scope drops the outgoing Congress 45 days into the new one (2027-02-17 for the 119th) while Table III lags about 8 months, so the 119th would stop being walked and the 120th's cold start would stick. Being resolved before January 2027, preferably by reading OLRC's Table III bulk archive instead of the page chain (receipt `table3-bulk-2026-09-26/`).
