...
- **Courts and other sources:** the court citation tables and opinion index; CRS reports; FCC proceedings and filings; the bounded lobbying load (decision 10); and, since 2026-09-26, every active SAM registration year from 1996 plus the 1949 and 1968 outliers (its row).
...
- **Partial.** `congress_bills` lacks bills that the retained 110th–113th
  ...
  their rows state. FCC filings, lobbying, CRS and court dockets have no
  retained source responses in their producing runs, so only publication
  integrity is verified. (SAM has since retained its extracts, with credits,
  retirements and abandoned triggers journaled; see its row.)
...
| T16 | `run-rollup-congress-bills` (retired, A1) | `congress_bills.parquet` | qualified at `6849376e…` (2026-09-26): congress_bills holds 421,465 bills. BILLSTATUS lists 134,432 for the 108th–117th, and all are held. `117-hr-11`, which the earlier reader refused in the 117th House archive, is now read at BILLSTATUS schema 1.0.0 (SpicyDocs 0.41.0), and no identity of `9f861d50…` is lost. 54 list-era rows for reserved House numbers BILLSTATUS no longer lists are retained. All 513 bills the drift audit found missing in the 110th–113th are present, no identity of `8ab2a2cf…` is lost, and no key repeats. bill_committee_actions → congress_bills misses only the publisher's misprint `118-hr-14106`. Receipts: `congress-bills-backfill-2026-09-26/`, `join-gaps-2026-09-26/d/`. |
...
| T14 | `run-rollup-sam-entities` | `sam_entities.parquet` | qualified at `9a7de1ac…` (2026-09-26): the backfilled, retiring registry, 792,846 active registrations over registration years 1949, 1968 and 1996–2026, no row undated. Each unbounded year replaces its rows only after its extract meets its declared count. The generation audit reconciles the removals exactly: 80 retired in 2002–2003 at `0ed551d3…`, 11 in 2026 here, each named in a `rows-retired` event, none unjournaled and none still present. The 8 renewals SAM holds Active twice are credited, each in a `sam-superseded-credited` event dated before its trigger's cutoff. Evidence is admitted and bound, and no credential appears among 10 configured secrets. Counts reconcile: SAM reports 793,040 active (per the reconciliation receipt's entity-API query), which is 792,846 held plus 8 credited, 181 activated after their year was read (the current year is re-read every other day), and 5 registrations with no registrationDate. A year-windowed extract cannot reach those 5, so they are a known exclusion. Rows held per year match an independent recount of the published table. Field values were not re-sampled at this pin. Receipts: `sam-entities-0ed551d3-2026-09-26/`, `sam-entities-9a7de1ac-2026-09-26/`, `sam-entities-reconciliation-2026-09-26/per-year.json`. |
...
| T09 | `run-rollup-bill-family` | `congress_bills.parquet`, `bill_actions.parquet`, `bill_committees.parquet`, `bill_publisher_summaries.parquet`, `bill_versions.parquet`, `bill_sections.parquet`, `section_diffs.parquet`, `section_diff_items.parquet`, `financial_changes.parquet`, `section_classifications.parquet`, `bill_summaries.parquet`, `diff_summaries.parquet`, `cbo_cost_estimates.parquet`, `public_activity_events.parquet`, `bill_family_archives.parquet`, `bill_vote_references.parquet`, `bill_family_backfills.parquet`, `bill_family_backfill_walks.parquet` | qualified at `6849376e…` (2026-09-26): the BILLS bulk text backfill (9 runs, 113th–119th, SpicyDocs 0.41.0, receipt `bill-family-bulk-2026-09-26/final/`) extends the family audited at `9f861d50…`. 133,862 printings gained a body: 137,410 of the 137,641 listed in the 113th–119th have one (99.8%), and the rest are not XML or were refused, each recorded. `bill_sections` holds 2,659,863 rows, and every printing's stated section count matches them. 140 bulk-read printings, 20 per Congress, fetched one by one from GovInfo's per-package links, equal their recorded bytes. Against `9f861d50…`, no table loses an identity except `bill_versions`. Its 136,743 removals are all listing-only placeholders whose version now has its body. Two public-law placeholders (113-hr-1067, 113-hr-1068) remain beside their bodies, a known defect. Keyless tables differ only in re-observation times, and the generation audit finds 0 failures. At `9f861d50…`: the status-only backfill (10 runs, 108th–117th, `max_version_fetches=0`, receipt `congress-bills-backfill-2026-09-26/`) extends the family audited at `8ab2a2cf…`, described below. Across the chain, no version, section, diff, committee or action row of `8ab2a2cf…` is lost. It adds 1,298 bills, 763,098 actions, 180,077 metadata-only printings and a one-off 314,496 public-activity events, and every step's audit is clean (the last: 0 findings). Earlier printings of the 108th–117th are metadata only, with no sections or diffs, until their bodies are fetched. At `8ab2a2cf…`: SpicyDocs 0.37.0/0.38.0 (runs 36227172365 for the 119th and 36230043425 for the 118th, capped at 10 printing fetches) repair every defect the drift audit and its re-audits found. No pair runs backward; a dateless enrolled printing follows its dated predecessors. `bill_sections` is keyed on `seq` with no duplicate, and 119-hr-5334 enrolled, 119-hr-9022 reported and 119-hr-4060 introduced publish every section. A numbered reprint keeps its own code: 119-hr-6644's `eas` (198 sections) and `eas2` (276) are separate and chain placed-on-calendar → eas → engrossed-amendment-house → eas2 → enrolled, matching independent native parses of both documents; 118-hr-7643's `rh2` is separate, and no row keys a reprint's package under another code. Every diff item resolves to a published section. Each generation's removals are accounted for (32 stale pairs, then 119-hr-6644's 78 misfiled sections, stale row and 2 pairs; keyless tables differ only in re-observation times). The generation audit finds 0 failures, with evidence `18ea08d4…` admitted and bound. Limits: the four model outputs stay uncomputed, and historical bodies before the current Congress are only partly captured. Receipts: `bill-family-d380cdc0-2026-09-26/`, `bill-family-rerun-2026-09-26/`, `bill-family-8ab2a2cf-2026-09-26/`, `repeated-printings-2026-09-26/`. |
...
| T08 | `run-rollup-amendments` | `amendments.parquet` | qualified at `94c5868b…` (2026-09-26): 7,106 rows. R8 admits evidence `4aee7cd8…` (51 members, bound, 0 failures). All 48 rows of the retained 09-21…09-26 window (one list page declaring 48, and 48 details) replay cell for cell through an independent raw-JSON parser: 864 of 864 match. The other 7,058 rows equal the qualified `af5ef4c3…`, and no identity of it or of `559fd5f4…` is lost. SA 6782's missing 09-23 action is a Congress.gov restatement: the retained responses carry the publisher's 09-25 11:08Z stamps, and a native read at 09-26 12:25Z shows the same stamp and no 09-23 action. Receipt: `amend-hc-qualification-2026-09-26/amendments/`. |
...
| T08 | `run-rollup-roll-call-votes` | `roll_call_votes.parquet`, `member_votes.parquet` | qualified at `9072b65d…` (2026-09-26): the 108th–119th, 23,358 roll calls and 7,384,694 positions, with no identity of `45b8f6b8…` removed. House 108th–118th holds 14,503 of the Clerk's 14,509 listed roll calls and Senate 7,276 of 7,277; the 7 absences are each stated by the source. 108-1-2's totals contradict its members' choices; five House files (112-1-484, 112-2-327, 113-2-275, 114-1-300, 114-2-44) list no recorded votes, and the two opened say "vacated by unanimous consent"; the Senate menu withholds 116-2-216 (a secret session, journaled `vote-withheld`). Links come from the bill's recorded action, then the vote file's own statement (`vote_file_legislation`). Every vote join holds: member_votes.bioguide_id misses only L000555, Luke Letlow, who died before taking his seat. The generation audit finds 0 failures; its 13 reviews are the Clerk's HTML index pages at the vote-source stage. Receipt: `votes-backfill-2026-09-26/final/`. At `45b8f6b8…`: 1,579 votes and 382,536 positions, every prior identity retained. Only Senate 212/213 `match_action_index` change (21→17, 19→15) under `recorded-vote-first-numeric-action-v2`; all 845 links equal the first recorded reference in bill-family `5990abbb…`, 734 remain unmatched, and `member_votes` is byte-identical. All 50 retained overlap XMLs match every roll-call and member cell. Other votes inherit `0255d5de…` by exact equality; wider history and scorecards remain open. Receipt: `repair-qualification-2026-09-25/votes-usaspending/`. |
...
| T08 | `run-rollup-laws` | `table3_records.parquet` | qualified at `a6ff3892…` (2026-09-26): table `38d110cd…`, 2,983 rows over 40 acts, derived from OLRC's Table III bulk file at release point 119-73. It equals the replay of the bulk derivation row for row on `(act_key, seq)`, in every column but `observed_at`. That replay was compared with the page-walk table `fd1609ad…`, whose rows had replayed cell for cell against their retained pages on 2026-09-25. All 2,981 of those rows survive, with four act-level columns (`stated_key`, `congress`, `act_date`, `statutes_at_large_volume`) restated in the file's spelling. 119-30 and 119-53 are added; the walk never reached them, and each was probed against its OLRC page. The R8 audit pairs all 2,981 prior identities, removes none and fails nothing. Receipt: `table3-bulk-2026-09-26/` (`live/`). |
...
| T08 | `run-rollup-house-communications` | `house_communications.parquet` | qualified at `fa33e547…` (2026-09-26): 5,006 rows, 0 list-only. R8 is clean on it and on each generation of the 09-26 evidence drain (`bc3715f0…` → `fa33e547…`, runs 36243144053–36248685095 at `a663f38`): evidence admitted and bound, 0 failures. The journaled unverified remainder falls from 4,006 to 0. List membership (5,006 = the declared count of its 43 retained pages) and all 40,048 list cells replay. Every row's detail replays cell for cell from the one retained response that read it, through an independent raw-JSON parser: 170,204 cells, 0 mismatches. Against `257e889a…` only the six formerly list-only rows changed; all 5,000 re-read details came back identical. RIN limit: the rule misses 14 report natures whose RIN is malformed (11) or labelled `RINs:`/"Regulation Identification Number" (3), and keeps only the first RIN on the 6 rows that state several. Receipt: `amend-hc-qualification-2026-09-26/house-communications/final/`. |
...
- **Rulemaking joins:** the September 25 local refresh published `snapshot_6d3dc0f2…`, including the merged labelled-docket and decisions 32–33 corrections. Its public manifest and every output match the local bytes; it holds 268,314 proceedings after the completed metadata catch-up. That snapshot is superseded: R5 is met and rulemaking qualifies at `snapshot_911969b4…` (its row; receipt `r5-reaudit-2026-09-26/`); the earlier reviewed scope is retained in `docket-lists-2026-09-24/published-step/`.
...
- **Wider populations:** lobbying beyond its bounded load; SAM's 5 registrations with no registrationDate, which a year-windowed extract cannot reach; FCC filings before the first window; USAspending beyond its top-100-page selection; CourtListener's full scope, rate budgets, newer catch-up and the non-civil docket scope decision (log: Scope and evidence gaps, Latest continuation).
- **Lifecycles:** a pairing and unknown-docket policy before `rulemaking_lifecycles` returns (decision 4).
- **Reports and hearings:** the family qualifies at `95810b26…` (its rows); the compiled-volume date is fixed (CHRG-117shrg56721 publishes no scalar date and all 11 native dates). The two 1946 historical parts (`CHRG-79jhrg79716p11`, `p19`) are admitted by the grammar but refused over the 24 MiB body bound (312 and 373 MB, PDF-only); a size refusal is being made final so they stop being re-requested. The earlier [report refresh](#september-24-report-refresh) remains dated history.
- **Successful-run findings:** fixed and verified live on 2026-09-26: private laws 119-1 and 119-2 read `captured_partial` with their bytes kept; Table III keeps its four blank-act-section rows (act 119-37); USAspending dates 10,000 of 10,311 rows. The broader subjects audit remains partial. SAM now retains every extract it reads: the 2026 file and each year the scheduled rotation reads. Ordinary external readers need routine source-response retention under T18. See the [log audit](rollup-success-log-audit-2026-09-24.md) and [September 25 audit](parallel-rollup-audit-2026-09-25.md).
- **FEC (T05):** broader individual records, committee history and correction streams (log: Scope and evidence gaps).
- **Table III bulk (2026-09-26):** `table3_records` is now derived from OLRC's Table III bulk file, one request a run, for every Congress the laws table holds, so the chain cold start and the 45-day scope window no longer apply. The first bulk generation (laws `a6ff3892…`) restates four act-level columns in the file's spelling and adds 119-30 and 119-53, which the page walk never reached; T08 records its re-qualification (receipt `table3-bulk-2026-09-26/live/`).
