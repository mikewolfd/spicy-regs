...
- **Table III bulk (2026-09-26):** `table3_records` is now derived from OLRC's Table III bulk file, one request a run, for every Congress the laws table holds, so the chain cold start and the 45-day scope window no longer apply. The next generation restates four act-level columns in the file's spelling and adds 119-30 and 119-53, which the page walk never reached; it needs re-qualification against `fd1609ad…` (receipt `table3-bulk-2026-09-26/`).
