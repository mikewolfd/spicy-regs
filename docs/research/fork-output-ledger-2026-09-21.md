...
| T16 | `run-rollup-congress-bills` (retired, A1) | `congress_bills.parquet` | current `55671b43…` is PARTIAL (2026-09-26): no prior identity is removed and 188 are added, and all 513 bills that retained 110th–113th BILLSTATUS lists are still absent. Field checks against fresh BILLSTATUS were not rerun at this pin; at `5990abbb…` 150 of 166 changed rows matched in eight fields. Scoped repair remains qualified at `4a1949a9…` (2026-09-24). Receipts: `bill-family-55671b43-2026-09-26/`, `drift-qualification-2026-09-26/bills-citations/`. |
...
| T09 | `run-rollup-bill-family` | `congress_bills.parquet`, `bill_actions.parquet`, `bill_committees.parquet`, `bill_publisher_summaries.parquet`, `bill_versions.parquet`, `bill_sections.parquet`, `section_diffs.parquet`, `section_diff_items.parquet`, `financial_changes.parquet`, `section_classifications.parquet`, `bill_summaries.parquet`, `diff_summaries.parquet`, `cbo_cost_estimates.parquet`, `public_activity_events.parquet`, `bill_family_archives.parquet`, `bill_vote_references.parquet`, `bill_family_backfills.parquet`, `bill_family_backfill_walks.parquet` | current `55671b43…` is FAILED (2026-09-26): scheduled run 36211271899 built it with code that predates the ordering and identity repairs, so both defects persist. `section_diffs` holds 18 pairs from a dateless enrolled printing back to the introduced one (13 at `5990abbb…`), and 119-hr-5334 enrolled and 119-hr-9022 reported still lack one section each. The generation audit (`scripts/audit_generation.py`) admits every member byte and finds every keyed table unique, with no prior identity removed. Keyless `bill_vote_references` and `bill_family_archives` differ only in re-observation times, plus 8 new vote references. It admits the first retained BILLSTATUS evidence, `65415e8f…`, bound to this generation: 1,816 captures, every body of its expected kind, and no credential in evidence or table cells. Source qualification is not assessed at this pin; at `5990abbb…` all 147 text diffs reconstructed from independent XML reads. The four model outputs remain uncomputed. Receipts: `bill-family-55671b43-2026-09-26/`, `drift-qualification-2026-09-26/bills-citations/`. |
