...
- **Votes:** the local House candidates contain 577 roll calls and 249,099 member-vote rows; the bounded local audit is complete. The confirmed selection defect is repaired and independently reviewed: source identity is enumerated independently of optional bill links, and the host now uses complete Senate menus. Unseen votes precede refreshes, each chamber receives overlap, and only successful vote reads replace their member rows. The newly retained House identity census contains every prior key plus exactly 99 unlinked keys; the old original listing was not retained, so source stability across the two observations remains unproved. Raw listing/body retention and whole-selection qualification remain required before publication or full-history scorecards. All 445 retained House voters join the qualified crosswalk, but eleven member/term-date joins remain unresolved. The capture driver and complete retained enumeration are now independently approved: six responses establish 1,573 unique votes (House 362/314 and Senate 659/238, sessions 1/2), including all 99 House identities without optional bill links. Offline replay sends no new requests and reproduces the selection. Bounded native XML acquisition is active. The first batch retains a valid House Speaker-election file (`119:house:1:2`) refused by the existing reader: it uses candidate totals and literal candidate-name choices. All 434 choices reconcile independently to its stated counts; source support and table preservation are being reviewed. Whole-output qualification remains open. See `votes-qualification/`.
...
- **Court clusters:** the complete docket map is qualified: 71,677,647 unique IDs and exact full native semantic-digest agreement, with no missing IDs/courts. The dependent worker is streaming the complete cluster edition using pinned local inputs. Its first preflight refused one source court with blank jurisdiction; a reviewed gate now proves that no docket references that court before allowing it. All native court rows remain retained. Local controls, worker plans and this narrow gate passed independent review. All 10,070,727 retained cluster identities and 36 old fields must be compared before qualification. The existing generic incremental merge has schema/freshness limitations; this edition build bypasses it. Full real output qualification and publication remain pending. See `courtlistener-clusters-qualification/HANDOFF.md`.
...
review. Its September 22, 2026, 03:24 UTC checkpoint distinguishes completed
...
`b0a8e6e` passed GitHub checks. The later 0.26.2 adoption is committed at `9c58095`: source/main and isolated gates pass 7,308 tests; the installed host passes 2,411 tests. Only the two reviewed vote/press interpretation modules changed in the wheel. The
