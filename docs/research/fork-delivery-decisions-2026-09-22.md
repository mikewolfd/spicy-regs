...
| 33 | Whether every FR document merges all the dockets it names into one proceeding (which built a 483-docket FDA and a 412-docket FMCSA proceeding from RIN-less notices). | **Only an FR document with a RIN or a rule stage unions the dockets it names; other documents attach to each named docket's proceeding as a reference. A comment period whose notice names dockets now held by several proceedings lists every one of them in `proceeding_ids_json` (amended 2026-09-24 after review: 954 periods had picked none).** | The glue is not rulemaking: of 2,467 multi-docket documents, 1,642 are RIN-less Notices (FMCSA driver-exemption applications naming up to 100 dockets; FDA information-collection approvals, 449 of 483 dockets Nonrulemaking); a size cap has no natural gap (2→100 continuous; K=2 still leaves 14-docket chains and discards NPL rules). With the gate, multi-docket proceedings fall 1,002 → 435 and their dockets 4,202 → 1,260; the residual 56- and 43-docket EPA proceedings are National Priorities List rules over all-Rulemaking dockets, a genuine series. Blind ruling 2026-09-24. Confirmed by Mike 2026-09-24; merged with 32. |
