...
| 32 | Which Regulations.gov dockets are rulemaking action dockets in `proceedings`, given that `"rulemaking" in docket_type` also matches `Nonrulemaking`. | **A docket is an action docket only when it states a RIN or its `docket_type` is exactly `Rulemaking`; the 157,402 Nonrulemaking shells (no FR document, stage event, agenda link or RIN) leave `proceedings`, and their comment periods stay docket-anchored.** | The dockets table holds exactly `Rulemaking` (63,127; 62,791 with a RIN) and `Nonrulemaking` (215,997; none with a RIN); the shells are FDA civil-money-penalty orders, FAA and DOT-OST exemptions, permits, meetings and information collections, each a single-docket proceeding duplicating its docket row; the pipeline defines a proceeding as a rulemaking with its actions, and no consumer reads `proceedings` yet. Effect: proceedings 458,884 → 301,482; `rule_targets` and agenda links unchanged; 83,608 comment periods keep their anchor but re-key. Blind ruling 2026-09-24. |
| 33 | Whether every FR document merges all the dockets it names into one proceeding (which built a 483-docket FDA and a 412-docket FMCSA proceeding from RIN-less notices). | **Only an FR document with a RIN or a rule stage unions the dockets it names; other documents attach to each named docket's proceeding as a reference.** | The glue is not rulemaking: of 2,467 multi-docket documents, 1,642 are RIN-less Notices (FMCSA driver-exemption applications naming up to 100 dockets; FDA information-collection approvals, 449 of 483 dockets Nonrulemaking); a size cap has no natural gap (2→100 continuous; K=2 still leaves 14-docket chains and discards NPL rules). With the gate, multi-docket proceedings fall 1,002 → 435 and their dockets 4,202 → 1,260; the residual 56- and 43-docket EPA proceedings are National Priorities List rules over all-Rulemaking dockets, a genuine series. Blind ruling 2026-09-24. |
