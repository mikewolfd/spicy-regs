...
| `roll-call-votes` | Each chamber's own index (Clerk EVS pages, Senate LIS menu), keyless, then Clerk and LIS files only for roll calls not already published with a tally (and, once, House rows published before `legis_num`), plus, for a sitting Congress only, the newest `OVERLAP_VOTES` re-read for corrections; held links are refreshed from each row's own columns | `MAX_VOTES_PER_RUN`, newest first; an earlier Congress is a dispatched scope, read once |
...
| `committee-rosters` | The unscoped `committee` list of every Congress, whole (four pages, 818 records on 2026-09-26; a per-Congress list over-declares and `walk_route` publishes what it served), then details only for committees not yet folded or whose list `update_date` moved, never for a code the detail route cannot spell; both chamber files, whole, each replacing its chamber's seats for the current Congress | `MAX_DETAILS_PER_RUN`, newest `update_date` first; pinned in `tests/test_committee_rosters.py` |
